...
use fields (
	...
	'carbon'                  # bool: true if message carbons enabled for this connection
);
...
sub to_host
{
	my $self = shift;
	return $self->{to_host}
	  or die "To host accessed before it was set";
}
...
sub version
{
	my $self = shift;
	return $self->{version}
	  or die "Version accessed before it was set";
}
...
sub event_read
{
	...
	eval { $p->parse_chunk_scalarref($bref); };
...
sub start_init_stream
{
	...
	# {=xml-lang}
	my $xml =
qq{<?xml version="1.0" encoding="UTF-8"?><stream:stream $to xmlns:stream='http://etherx.jabber.org/streams' xmlns='}
	  . exml($xmlns)
	  . qq{' xml:lang='en' $extra_attr $ver_attr>};
...
sub start_stream_back
{
	...
	my $ns   = delete $opts{'namespace'}
	  or die "No default namespace";    # {=stream-def-namespace}
	...
	# {=streams-namespace}
	my $back =
qq{<?xml version="1.0" encoding="UTF-8"?><stream:stream $from_attr id="$id" $ver_attr $extra_attr xmlns:stream="http://etherx.jabber.org/streams" xmlns="$ns">$features};
...
sub stream_error
{
	...
	{
		$self->write(
qq{<?xml version='1.0'?><stream:stream xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams' id='bye' xml:lang='en'>}
		);
	}
