...
sub available
{
	...
	my $xml = DJabberd::XMLElement->new( "", "presence", { '{}from' => $from->as_string, }, [] );
...
sub probe
{
	...
	my $xml = DJabberd::XMLElement->new(
		"",
		"presence",
		{
			'{}type' => 'probe',
...
sub _process_inbound_subscribe
{
	...
	{
		my $subd = DJabberd::Presence->make_subscribed(
			to   => $from_jid,
			from => $to_jid
		);
		...
		my $probe = DJabberd::Presence->probe(
			from => $from_jid,
			to   => $to_jid
		);
...
sub _process_outbound_unavailable
{
	...
	if ( !$skip_alter && $vhost->are_hooks("AlterPresenceUnavailable") )
	{
		...
		$vhost->run_hook_chain(
			phase   => "AlterPresenceUnavailable",
			args    => [ $conn, $self ],
			methods => {
				done => sub {
					return if $conn->{closed};
