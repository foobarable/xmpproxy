...
sub close
{
	...
	{
		$self->vhost->run_hook_chain(
			phase   => "ConnectionClosing",
			args    => [$self],
			methods => {
				fallback => sub {
				},
...
sub on_stanza_received
{
	...
	$DJabberd::Stats::counter{"ClientOut:$class"}++;
...
sub filter_incoming_client_builtin
{
	...
	if ( $from && !$self->is_authenticated_jid($from) )
	{
		...
		if (   $vhost->quirksmode
			&& $stanza->isa("DJabberd::IQ")
			&& $stanza->type eq "error"
			&& $stanza->from eq $stanza->to )
		{
