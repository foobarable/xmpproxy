...
sub new
{
	...
	$self->set_config_types("Message Presence");
	return $self;
}
...
sub on_initial_presence
{
	...
	my $from = $conn->bound_jid
	  or return;
	$logger->info( "Loaded offline stanzas for " . $from->as_bare_string );
	...
	foreach my $message (@$messages)
	{
		...
		$logger->info( "Deleted stored stanza " . $message->{id} . " for " . $stanza->to_jid()->as_bare_string );
...
sub deliver
{
	...
	$logger->info( "Stored stanza for " . $to->as_bare_string );
